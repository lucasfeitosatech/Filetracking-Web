...
module.exports = {
    ...
    async filetrackingPost(req,res,next){
        ...
        console.log("y bytes recebidos da imagem de entrada");
        console.log("y = " + x);
        res.send(create_qrcode(bytes));
    }
};

var create_qrcode = function(bytes) {
    var qr = qrcode(0, 'L');
    y =  bytes;
    id = crypto.createHmac('sha256', 'easytracking')
    .update(intArraytoString(bytes))
    .digest('hex');
    console.log("Id gerado dos bytes: " + id);
    var redu = encode2BitsInfo(y);
    console.log("Quantidade de bits da redundancia");
    console.log("bits da redundancia = " + redu.length);
    b = encode6BitsInfo(y,redu);
    console.log("Quantidade de bytes obtidos dos 6 bits de cada byte de y");
    console.log("b = " + b.length);
    
    var message = generateMessage(b,id,x);
    console.log("Quantidade de bytes que vão no QR Code sendo que 2 bytes são utilizados para a quantidade de bytes da informação 32 bytes para o ID e o restando são os 6 bits de cada de byte de y");
...
var encode2BitsInfo = function(y) {
    ...
    if (bits.length % 16 != 0) {
        var padding = 16 - bits.length % 16;
        bits = bits + "0".repeat(padding);
    }
    
    k = bitsArray2Int16(bits);
    console.log("Quantidade de k bytes obtidos a partir dos 2 primeiros bits de cada byte de y");
    console.log("k: " + k.length * 2);
    console.log("Quantidade de k símbolos para m = 16");
    console.log("k: "+ k.length)

    var redundancy = generateRedundancy(bits,k);
    var redu = bitsArray2Int(redundancy);
    console.log("Quantidade de símbolos para m = 16 da redundancia");
    console.log("n - k = " + redu.length);
...
var generateRedundancy = function(bits,k){
    ...
    var gf = new GenericGF(69643, 65536, 1);
...
var encode6BitsInfo = function(y,redu) {
    ...
    console.log("Quantidade de bits dos 4 primeiros bits de cada byte de y");
    console.log("bits dos 4 bits = " + x*4);
    console.log("Considerando um bits de cada byte como (b1,b2,b3,b4,b5,b6,b7,b8) ");
    console.log("Matematicamente os bits da redundancia sao sempre maiores ou igual aos 4(b3,b4,b5,b6) bits de cada byte de y, possibilitando então assim o OTP nesses bits com os bits da redundancia");
...
var bitsArray2Int16 = function(bits) {
    var bitCount = 0;
    var byte = "";
    var bytes = new Array();
    var byteCount = 0;
...
var generateMessage = function(b,id,x){
    var m = "";
    for(i=0;i<b.length;i++){
        var code = b[i];
        m = m + String.fromCharCode(code);
    }
    var bitsCount = x.toString(2);
    bitsCount = addPadding(bitsCount,16);
    var numBytes = bitsArray2Int(bitsCount);
    var charBytes = String.fromCharCode(numBytes[0] + 32,numBytes[1] + 32)
    var idBits = "";
